 ...
 *
 * Loads .env.local (for GEMINI_API_KEY) from the project root. Static imports
 * are safe because foodVision uses a lazy client that reads GEMINI_API_KEY at
 * call time (inside scanFoodImage), not at import time. Does NOT touch any live
 * flow.
 */
...
import { FOOD_CATEGORIES, type FoodScanResult } from "@/types/food";
